...
### Scenario 2: "안전한 지출/환급 설계 계획 세우기" (Backend)
> **목표**: 환급형 지출과 원장 모델이 꼬이지 않도록 active canonical 기준의 계획 문서 만들기
...
> **Command + Prompt**:
> ```text
> /pdca-plan expense-reimbursement
> "챌린지 지출 환급 흐름을 설계할 거야.
> 1. append-only 원장 기준으로 계획을 세워줘.
> 2. `expense_execution_sessions` 모델을 쓰고,
> 3. idempotency key, 보증금, 장부 정합성을 빠뜨리지 않게 해줘."
> ```

**AI는 이렇게 생각하고 계획을 잡습니다:**
1. `/pdca-plan` ➡️ active 기준 계획 문서 생성
2. `append-only 원장` ➡️ 금융 이벤트와 projection 구분
3. `expense_execution_sessions` ➡️ 환급형 지출 실행 세션 모델 적용
4. `idempotency key` ➡️ 중복 처리 방지 규칙 반영
...
| 상황 | 치트키 프롬프트 |
| :--- | :--- |
| **설계 검증** | "이 설계가 active canonical의 금융/상태 규칙을 준수하고 있어? append-only, idempotency, canonical enum 기준으로 리뷰해줘." |
| **디자인 적용** | "하드코딩된 색상(#123456) 다 빼고, WDS 디자인 토큰(`var(--color-...)`)으로 싹 바꿔줘." |
| **에러 해결** | "응답 모델이 canonical OpenAPI와 맞는지 봐줘. active public API 기준으로 정리해줘." |
...
### 🛠️ Backend Legacy Migration (Spring Boot / Django)
> 이 섹션의 명령어는 기본 active 흐름이 아니라 legacy migration 전용입니다.
